	...
	for (int k = 2; k < 15; k++) {
		...
		for (int i = 1; i < sensordata.size() - 1; i++) {
			...
			for (int j = 1; j < wifidata.size() - 1; j++) {
				double wifix = atof(wifidata[j][1294].c_str()), wifiy = atof(wifidata[j][1295].c_str());
				double dis = caldistance(wifix, wifiy, sensorx, sensory);
				if (dis < mindistance && atoi(wifidata[j][1296].c_str()) < 9 ) {
					mindistance = dis;
					minindex = j;
				}
			}
			if (mindistance <= 0.05 && minindex != -1 && minindex != 0) {
				wifidata[minindex][1296] = to_string(atoi(wifidata[minindex][1296].c_str()) + 1);
				sensordata[i].insert(sensordata[i].end(), wifidata[minindex].begin() + 3, wifidata[minindex].begin() + 753);
