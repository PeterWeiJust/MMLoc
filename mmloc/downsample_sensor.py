...
# -*- coding: utf-8 -*-
"""
Created on Sun Feb 16 21:40:15 2020
...
from sensor_data_functions import normalisation,overlap_data,read_overlap_data,downsample_data
...
class DownsampleDataset(torch.utils.data.Dataset):
    def __init__(self,train=12,val=13,tw=1000,slide=100,mode="train",datapath='timestep100/',labelpath='sensordata/sensor_wifi_timestep100_',transform=torch.from_numpy):
        self.mode=mode
        self.datapath=datapath
        self.labelpath=labelpath
        self.transform=transform
        self.trainx,self.trainy=downsample_data(1,train,tw,slide)
        #self.valx,self.valy=downsample_data(train+1,val)
        self.testx,self.testy=downsample_data(14,14,tw,slide)
...
train_sensor=DownsampleDataset(mode='train')
...
suf = "sensor_downsample"

# ---save the model---
torch.save(lstmmodel.state_dict(),  suf + ".pkl")

# ---load the model---
lstmmodel.load_state_dict(torch.load(suf+".pkl"))
lstmmodel.eval()


test_sensor = DownsampleDataset(mode='test')
test_loader = torch.utils.data.DataLoader(dataset=test_sensor, batch_size=len(test_sensor), shuffle=False)
sensor_test_set = torch.Tensor(test_loader.dataset.testx).view(len(test_loader.dataset.testx), 10,
